...
abstract class BaseModel
{
    ...
    //permet de mettre dans le where n'importe quel colone 
    public function findByColumn($columnName, $columnValue)
    {
        $req = "SELECT * FROM " . self::getClassName() . " WHERE $columnName = :value";
        $db = Database::getIntance()->db;
        $prepared = $db->prepare($req);
        $prepared->bindValue(':value', $columnValue);
    
        if ($prepared->execute()) {
            $results = $prepared->fetchAll(PDO::FETCH_ASSOC);
    
            if (!empty($results)) {
                // Si vous souhaitez traiter les résultats d'une manière particulière, vous pouvez le faire ici
                return $results;
            } else {
                return false; // Aucun résultat trouvé
            }
        } else {
            return false; // Gérer le cas où la requête échoue
        }
    }
