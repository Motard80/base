...
class Form
{
    ...
    /**
     * 
     */public function select($id, $name, $msgLabel, $SelectName, $ValCtrl, $ValBoucle, $IdSelect, $ValSelect)
{
    // Vérifier si $ValCtrl est un tableau ou un objet
    if (!is_array($ValCtrl) && !is_object($ValCtrl)) {
        // Si ce n'est pas un tableau ou un objet, afficher un message d'erreur
        return "Erreur: Le paramètre \$ValCtrl doit être un tableau ou un objet.";
    }

    $options = '';
    foreach ($ValCtrl as $item) {
        // Assurez-vous que $item est bien un tableau ou un objet avant d'essayer d'accéder à ses éléments
        if (is_array($item) || is_object($item)) {
            $options .= '<option value="' . $item[$IdSelect] . '">' . $item[$ValSelect] . '</option>';
        }
    }
    ...
    /*public function select($id, $name, $msgLabel, $SelectName, $ValCtrl, $ValBoucle, $IdSelect, $ValSelect)
    {
        return $this->surrond('<label for"' . $id . '">' . $msgLabel . '</label>'
            ...
            . '<?php foreach('.$ValCtrl.'as '.$ValBoucle.'){'
            .'<option value="<?='.$ValBoucle.[$IdSelect].'?>"><?= '.$ValBoucle.[$ValSelect].'?> '
            .'?>}');
    }*/
}
